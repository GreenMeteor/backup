...
class BackupManager
{
    ...
    private function backupDatabase($zip)
    {
        try {
            ...
            $addResult = $zip->addFile($dumpFile, 'database/db_dump.sql');
            if (!$addResult) {
...
private function createMysqlDump()
{
    ...
    try {
        ...
        // Attempt dump with config file method - more secure for passwords
...
private function dumpWithDirectCommand($mysqldumpPath, $tempFile, $host, $port, $dbName, $user, $password)
{
    try {
        ...
        // Handle password
        if (!empty($password)) {
            // Use MYSQL_PWD environment variable which is more secure than -p
...
public function restoreDatabase($sqlDumpPath)
{
    ...
    try {
        ...
        // Try restoring with config file first
        $backupDir = $this->getBackupDirectory();
        ...
        // If both methods fail, try native PHP approach
